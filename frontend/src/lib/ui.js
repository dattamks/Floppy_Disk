...
export const fmtStorage = (gb) => {
  ...
  if (gb >= 1024) { const tb = gb / 1024; return (tb >= 10 || tb % 1 === 0 ? Math.round(tb) : tb.toFixed(1)) + ' TB'; }
...
export const kindOf = (mime) =>
  mime.startsWith('video') ? 'video' : mime.startsWith('image') ? 'image' : mime.startsWith('audio') ? 'audio' : 'doc';
